...
Future<FlutterToolInfo> setUpFlutterTool({
  ...
  if (desiredEngineVersion == null) {
    throw CommandError(
      ...
      'This can happen if `haven create` or `haven upgrade` was interrupted, try deleting the environment with `haven rm ${environment.name}`',
    );
  }
  ...
  if (shouldPrecompile) {
    ...
    // Explicitly set the last accessed time so `haven gc` can figure out which
