...
Future<CommandMessage?> detectExternalFlutterInstallations({
  ...
  final havenFiles = await findProgramInPath(
    scope: scope,
    name: config.havenExecutableFile.basename,
  );

  final offending = {
    ...dartFiles.map((e) => e.path),
    ...flutterFiles.map((e) => e.path),
    ...havenFiles.map((e) => e.path),
  };

  offending.remove(config.havenDartShimFile.path);
  offending.remove(config.havenFlutterShimFile.path);
  offending.remove(config.havenExecutableFile.path);
  ...
  log.d('havenDartShimFile: ${config.havenDartShimFile.path}');
  log.d('havenFlutterShimFile: ${config.havenFlutterShimFile.path}');
...
Future<bool> tryUpdateWindowsPath({
  ...
  final env = <String, String>{
    'HAVEN_ROOT': config.havenRoot.path,
    if (config.legacyPubCache) 'PUB_CACHE': config.legacyPubCacheDir.path,
  };
