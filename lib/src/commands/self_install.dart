...
class SelfInstallCommand extends HavenCommand {
  SelfInstallCommand() {
    argParser.addFlag(
      'force',
      help: 'Overwrite an existing haven installation, if any',
      negatable: false,
    );
    argParser.addFlag(
      'promote',
  ...
  @override
  final name = 'install-haven';

  @override
  bool get hidden => true;

  @override
  final description = 'Finishes installation of the haven tool';
  ...
  Future<CommandResult> run() async {
    ...
    return withErrorRecovery(() async {
      final (result, profilePath) = await service.installHaven(
        scope: scope,
        runner: runner,
        force: force,
        promote: promote,
