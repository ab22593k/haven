...
class HavenVersion {
  HavenVersion({
    ...
    required this.havenExecutable,
  });

  final Version semver;
  final HavenInstallationType type;
  final HavenBuildTarget target;
  final Directory? packageRoot;
  final File? havenExecutable;
  ...
  static final provider = Provider((scope) async {
    ...
    final target = HavenBuildTarget.query();
    ...
    if (scriptFile.basename == 'puro.dart' &&
      ...
      if (projectRootDir.basename == 'global_packages') {
        installationType = HavenInstallationType.pub;
    ...
    if (_puroVersionDefine.isNotEmpty) {
      ...
      } else if (installationType == HavenInstallationType.pub) {
  ...
  static Future<HavenVersion> of(Scope scope) => scope.read(provider);
}

enum HavenBuildTarget {
  ...
  factory HavenBuildTarget.fromString(String str) {
    switch (str) {
      case 'windows-x64':
        return HavenBuildTarget.windowsX64;
      case 'linux-x64':
        return HavenBuildTarget.linuxX64;
      case 'darwin-x64':
        return HavenBuildTarget.macosX64;
      default:
...
Future<CommandMessage?> checkIfUpdateAvailable({
  ...
  final lastNotification = prefs.hasLastUpdateNotification()
      ...
      : null;
  final latestVersionFile = config.havenLatestVersionFile;
